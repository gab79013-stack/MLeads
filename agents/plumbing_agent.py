"""
agents/plumbing_agent.py
━━━━━━━━━━━━━━━━━━━━━━━
Plumbing Permits & Water Infrastructure Agent

Fuentes de datos:
  1. Socrata — Permisos de plomería filtrados por tipo
  2. NYC Open Data — Water main breaks & DEP permits (si aplica)
  3. EPA ECHO — Violaciones de agua potable (tratamiento → plumbing)

Detecta oportunidades de plomería para subcontratistas:
  - Repipe, water heater, gas line, backflow, sewer, water main
  - Nuevas construcciones con fase plumbing
  - Emergencias por breaks en water mains (trabajo de emergencia)

Intervalo default: 120 min
"""
...
# ── Socrata sources (plumbing-specific queries) ──────────────────────
SOCRATA_SOURCES = {
    ...
    "nyc":      {"domain": "data.cityofnewyork.us","dataset": "ipu4-2q9a", "city": "New York"},
    "honolulu": {"domain": "data.honolulu.gov",  "dataset": "m5a4-68h5", "city": "Honolulu"},
}

# Keywords que identifican permits de plomería
PLUMBING_KEYWORDS = [
    "plumb", "sewer", "water heater", "gas line", "backflow",
    "repipe", "water main", "gas pipe", "water pipe", "drain line",
    "water line", "plumbing", "sewer lateral", "grease trap",
    "water service", "sewer connection", "cross connection",
    "storm drain", "fire sprinkler", "domestic water",
]

# Excluir permits que son solo inspección o admin
EXCLUDE_KEYWORDS = ["re-inspection", "cancel", "expired", "void"]


class PlumbingAgent(BaseAgent):
    name      = "Plumbing Permits Agent"
    emoji     = "🔧"
    agent_key = AGENT_KEY
    ...
    def __init__(self):
        ...
        self.months    = int(os.getenv("PLUMBING_MONTHS", "6"))
    ...
    def _fetch_socrata(self, src_key: str, src: dict) -> list:
        ...
        # Build OR conditions for plumbing keywords
        keyword_clauses = []
        ...
        for r in rows:
            ...
            # Must match at least one plumbing keyword
            text = f"{desc} {ptype}"
            if not any(kw in text for kw in PLUMBING_KEYWORDS):
                continue
        ...
        logger.info(f"[{AGENT_KEY}] {src_key}: {len(leads)} plumbing leads")
        return leads
    ...
    def notify(self, lead: dict):
        ...
        msg = (
            "🔧 *PLUMBING* — " + scoring["grade_emoji"] + " " + scoring["grade"] + "\n"
        ...
        phone_str = lead.get("contact_phone", "")
        contractor_str = lead.get("contractor", "")

        msg = (
            f"🔧 *PLUMBING* — {scoring["grade_emoji"]} {scoring["grade"]}\n"
            f"📍 {lead.get(address, )}\n"
            f"🏙️ {lead.get(city, )}\n"
            f"📋 {lead.get(description, )[:120]}\n"
            f"🏷️ Tipo: {lead.get(permit_type, )}\n"
            f"💰 Valor: {value_str}\n"
            f"🏗️ GC: {contractor_str}\n"
            f"📞 {phone_str}\n"
