"""
agents/paint_agent.py
━━━━━━━━━━━━━━━━━━━━
Painting & Coating Permits Agent

Fuentes:
  1. Socrata — Permisos de pintura, stucco, repintado
  2. Lead-based paint disclosures (EPA RRP) — pre-1978 buildings
  3. HOA violation data (donde disponible) — exterior repaint requirements

Detecta oportunidades de pintura para subcontratistas:
  - Interior/exterior paint, repaint, stucco, primer
  - Commercial repaint, HOA compliance
  - Lead paint remediation (EPA RRP rule)
...
PAINT_KEYWORDS = [
    "paint", "painting", "repaint", "painter", "stucco",
    "primer", "exterior paint", "interior paint", "paint coat",
    "coating", "waterproofing", "sealant", "epoxy coating",
    "wall covering", "wallpaper", "venetian plaster",
    "lead paint", "abatement", "encapsulation",
]

EXCLUDE_KEYWORDS = ["re-inspection", "cancel", "expired", "void", "traffic paint", "road paint", "lane marking"]


class PaintAgent(BaseAgent):
    name      = "Painting & Coating Agent"
    emoji     = "🎨"
    agent_key = AGENT_KEY
    ...
    def __init__(self):
        ...
        self.months    = int(os.getenv("PAINT_MONTHS", "6"))
    ...
    def _fetch_socrata(self, src_key: str, src: dict) -> list:
        ...
        for r in rows:
            ...
            if any(ex in desc or ex in ptype for ex in EXCLUDE_KEYWORDS):
                continue
            if not any(kw in text for kw in PAINT_KEYWORDS):
                continue
        ...
        logger.info(f"[{AGENT_KEY}] {src_key}: {len(leads)} paint leads")
        return leads
    ...
    def notify(self, lead: dict):
        ...
        msg = (
            "🎨 *PAINT* — " + scoring["grade_emoji"] + " " + scoring["grade"] + "\n"
        ...
        msg = (
            f"🎨 *PAINT* — {scoring[grade_emoji]} {scoring[grade]}\n"
            f"📍 {lead.get(address, )}\n"
            f"🏙️ {lead.get(city, )}\n"
