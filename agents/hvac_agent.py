"""
agents/hvac_agent.py
━━━━━━━━━━━━━━━━━━━━
HVAC & Mechanical Permits Agent

Fuentes:
  1. Socrata — Permisos HVAC/Mechanical filtrados
  2. EPA ECHO — Violaciones de calidad del aire (→ HVAC upgrades)
 3. DOE Better Buildings — Benchmarking de eficiencia (→ replacements)

Detecta oportunidades de HVAC para subcontratistas:
  - Furnace, AC, ductwork, heat pump, mini-split, ventilation
...
HVAC_KEYWORDS = [
    ...
    "ductwork", "mini split", "heat pump", "ventilation",
    "ac unit", "air handler", "mechanical", "chiller",
    "boiler", " rooftop unit", "package unit", "split system",
    "refrigerant", "compressor", "condenser", "evaporator",
    "exhaust fan", "makeup air", "vrf", "variable refrigerant",
]
...
class HVACAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self.months    = int(os.getenv("HVAC_MONTHS", "6"))
    ...
    def _fetch_socrata(self, src_key: str, src: dict) -> list:
        ...
        logger.info(f"[{AGENT_KEY}] {src_key}: {len(leads)} HVAC leads")
        return leads
    ...
    def notify(self, lead: dict):
        ...
        msg = (
            "❄️ *HVAC* — " + scoring["grade_emoji"] + " " + scoring["grade"] + "\n"
        ...
        msg = (
            f"❄️ *HVAC* — {scoring[grade_emoji]} {scoring[grade]}\n"
            f"📍 {lead.get(address, )}\n"
            f"🏙️ {lead.get(city, )}\n"
